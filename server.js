...
app.get('/', async (req, res) => {
    try {
        ...
        const activeContact = contactRes.rows[0] || { phone: '', email: '', address: '' };
        res.render('dashboard', { products: prodRes.rows, contactInfo: activeContact, activeTab: 'shop', error: null, success: null });
    } catch (e) { 
...
app.get('/profile', async (req, res) => {
    ...
    try {
        ...
        const contactRes = await pool.query("SELECT * FROM contact_info WHERE id=1");
        const activeContact = contactRes.rows[0] || { phone: '', email: '', address: '' };
        res.render('dashboard', { products: [], contactInfo: activeContact, activeTab: 'profile', account: userRes.rows[0], customerOrders: orderRes.rows, error: null, success: null });
    } catch (e) { 
        res.status(500).send(e.toString()); 
    }
});
...
app.post('/profile/update', async (req, res) => {
    ...
    try {
        ...
        if (userCheck.rows.length === 0) return res.render('dashboard', { products: [], contactInfo: activeContact, activeTab: 'profile', account: { email: req.session.user.email }, customerOrders: [], error: 'Identifier footprint mismatch.', success: null });
        if (verificationCode !== '1234') return res.render('dashboard', { products: [], contactInfo: activeContact, activeTab: 'profile', account: userCheck.rows[0], customerOrders: [], error: 'Verification code invalid.', success: null });
        ...
        res.render('dashboard', { products: [], contactInfo: activeContact, activeTab: 'profile', account: userCheck.rows[0], customerOrders: orderRes.rows, error: null, success: 'Credentials updated successfully!' });
    } catch (e) { 
        res.status(500).send(e.toString()); 
    }
});
...
app.get('/checkout', async (req, res) => {
    ...
    try {
        ...
        res.render('dashboard', { 
            cart, contactInfo: activeContact, activeTab: 'checkout', error: null, success: null,
