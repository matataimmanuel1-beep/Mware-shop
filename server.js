...
app.post('/profile/update', async (req, res) => {
    ...
    try {
        ...
        if (userCheck.rows.length === 0) {
            return res.render('dashboard', { products: [], contactInfo: singleContact, activeTab: 'profile', account: { email: req.session.user.email }, customerOrders: [], error: 'Identifier footprint mismatch.', success: null });
        }
        if (verificationCode !== '1234') {
            return res.render('dashboard', { products: [], contactInfo: singleContact, activeTab: 'profile', account: userCheck.rows[0], customerOrders: [], error: 'Verification code invalid.', success: null });
        }
...
app.post('/login/admin', async (req, res) => {
    const { email, password } = req.body;
    try {
        const found = await pool.query("SELECT * FROM users WHERE email=$1 AND role='admin'", [email]);
        if (found.rows.length > 0 && found.rows[0].password === password) {
...
app.get('/admin', async (req, res) => {
    ...
    try {
        ...
        const contactRes = await pool.query("SELECT * FROM contact_info WHERE id=1");
        const singleAdmin = adminRes.rows.length > 0 ? adminRes.rows[0] : { email: 'admin@mwareshop.com', phone: '', password: '' };
        const singleContact = contactRes.rows.length > 0 ? contactRes.rows[0] : { phone: '+254 700 000 000', email: 'support@mwareshop.com', address: 'Mombasa, Kenya' };
        res.render('admin', { products: productsRes.rows, contactInfo: singleContact, adminProfile: singleAdmin, orders: ordersRes.rows, accounts: accountsRes.rows });
    } catch (e) { 
        res.status(500).send(e.toString()); 
    }
});
...
app.post('/admin/product/add', async (req, res) => {
    ...
    try {
        await pool.query("INSERT INTO products (id, name, price, currency, status, image) VALUES ($1,$2,$3,$4,$5,$6)", 
            [Date.now(), name, parseFloat(price), currency, status, image]);
        res.redirect('/admin');
    } catch (e) { 
        res.status(500).send(e.toString()); 
    }
